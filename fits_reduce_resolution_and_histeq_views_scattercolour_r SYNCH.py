...
intmap = hp.read_map(input_filename, field=1, nest=False)
print '-'*40

nside = 2**8 #default 2**11
nbr_bins = 1000 #no. of bins
nbr_times = 2 #no of times of histogram equalising
for method_name in ['root_4']: #'hist', 'linear', 'squared', 'root_2', 'root_3','root_4','root_5', 'root_6', 'log']:
    ball_pivoting = False
    r = 40 #unit in mm
    ...
    ary = np.transpose(ary)
    np.savetxt('SYNCH_nside%s_nbr%s__nbrtimes%s_r%s_sr%s_method%s_r.xyz' %(nside, nbr_bins, nbr_times, r, scale_range, method_name), ary)  
