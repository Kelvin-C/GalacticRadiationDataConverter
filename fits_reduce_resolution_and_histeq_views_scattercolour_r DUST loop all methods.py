...
input_filename = "COM_CompMap_ThermalDust-commander_2048_R2.00.fits"
...
intmap = hp.read_map(input_filename, field=0, nest=False)
print '-'*40

nside = 2**8 #default 2**11
nbr_bins = 1000 #no. of bins
nbr_times = 2 #no of times of histogram equalising
for method_name in ['hist', 'linear', 'squared', 'root_2', 'root_3','root_4','root_5', 'root_6', 'log']:
    #method_name = 'squared'
    ball_pivoting = False
    r = 40 #unit in mm
    scale_range = 80 #unit in mm
    threeD_model = False
    fourviews = False
    print 'Reformat:'
    ...
    np.savetxt('DUST_nside%s_nbr%s__nbrtimes%s_r%s_sr%s_method%s_r.xyz' %(nside, nbr_bins, nbr_times, r, scale_range, method_name), ary)  
...
if method_name == 'hist' and ball_pivoting == True:
    ...
    for i in range(len(ary)-1): #next point distance
        ...
        piv2_lst += [temp]
    #piv2_lst.sort()
    print 'pivoting_ball_radius2 lies below: ',  2*min(piv2_lst) #max(piv2_lst)
...
#plt all views
    plt.figure(3)
    plt.figure(3).suptitle("Dust View", fontsize="x-large")
